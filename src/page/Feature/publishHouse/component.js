...
export default function PublishHouse(props) {
  ...
  useEffect(() => {
    storage.get('code').then(res => {
      ...
      setHouseTypeList(setRadioProps(res.house_type));
  ...
  function handlerPublish() {
    ...
    // 出租要求,亮点
    // result.append('houseAddition.requirements', requirements);
    // result.append('houseAddition.spots', spots);

    // result.append('houseAmenity.items', items);
