...
function TabView(props) {
    ...
    const _setTabsChangeAddress = (item) => {
        ...
        let len = data.length - selectIndex - 1 > 0 ? data.length - selectIndex - 1 : 0
    ...
    const _renderTab = () => {
        return (
            <CityTab changeSelectId={(i) => setSelectIndex(i)}
                options={tabs} selectIndex={selectIndex} />)
    }
    return (
        <View style={[styles.container, style]}>
            <ScrollAbleTabView
                ...
                <CitySelect
                    ...
                    address={address}
                    changeAddress={(item) => { _setTabsChangeAddress(item) }} />
            </ScrollAbleTabView>
        </View>
    );
}
